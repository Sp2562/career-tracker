...
export default function App(){
  ...
  const persist = useCallback((d, t, ol)=>{
    if(!user) return;
    if(saveTimeout.current) clearTimeout(saveTimeout.current);
    setSyncStatus("saving");
    saveTimeout.current = setTimeout(async()=>{
      try {
        await saveData(user.uid, { done:d, times:t, openLv:ol });
        setSyncStatus("saved");
  ...
  // ── MAIN APP ──
  return (
    <div style={{fontFamily:"DM Mono, monospace",background:"#0A0A0A",minHeight:"100vh",color:"#E4E4E7"}}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Mono:wght@300;400;500&display=swap" rel="stylesheet"/>

      {/* TOP BAR */}
      <div style={{position:"sticky",top:0,zIndex:50,background:"#0A0A0A",borderBottom:"1px solid #27272A",padding:"10px 16px",display:"flex",alignItems:"center",gap:10}}>
        <div style={{flex:1}}>
          <div style={{fontSize:13,fontWeight:500,color:"#FAFAFA"}}>career.track</div>
        ...
        <div style={{display:"flex",background:"#18181B",borderRadius:8,border:"1px solid #27272A",overflow:"hidden"}}>
          {["tasks","stats"].map(p=>(
            <button key={p} onClick={()=>setPage(p)} style={{fontSize:11,padding:"5px 14px",background:page===p?"#27272A":"transparent",color:page===p?"#FAFAFA":"#71717A",border:"none",cursor:"pointer",fontFamily:"inherit",letterSpacing:"0.04em"}}>{p}</button>
          ))}
        </div>
        <button onClick={logOut} style={{fontSize:11,padding:"5px 10px",background:"transparent",border:"1px solid #27272A",borderRadius:7,color:"#71717A",cursor:"pointer",fontFamily:"inherit"}}>sign out</button>
      </div>
      ...
      <div style={{padding:"14px 16px 0"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:12}}>
          {[["done",`${totalDone}/${totalTasks}`],["progress",`${pct}%`],["time",fmtHuman(totalSecs)],["xp",`${totalDone*100}`]].map(([label,val])=>(
            <div key={label} style={{background:"#18181B",border:"1px solid #27272A",borderRadius:8,padding:"8px 10px"}}>
              <div style={{fontSize:9,color:"#52525B",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:3}}>{label}</div>
              <div style={{fontSize:14,fontWeight:500,color:"#E4E4E7"}}>{val}</div>
            </div>
          ))}
        </div>
        <div style={{height:3,background:"#27272A",borderRadius:2,marginBottom:14}}>
      ...
      {activeTimer&&(()=>{
        ...
        return(
          <div style={{margin:"0 16px 12px",background:"#052e16",border:"1px solid #166534",borderRadius:8,padding:"8px 12px",display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:8,height:8,borderRadius:"50%",background:"#22C55E",animation:"pulse 1s infinite"}}/>
            <div style={{flex:1,fontSize:11,color:"#86EFAC",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t?.name}</div>
            <div style={{fontSize:16,fontWeight:500,color:"#22C55E",fontVariantNumeric:"tabular-nums"}}>{fmtTimer(getLive(activeTimer))}</div>
            <button onClick={()=>stopTimer(activeTimer)} style={{fontSize:11,padding:"3px 10px",background:"#166534",border:"1px solid #22C55E",borderRadius:6,color:"#22C55E",cursor:"pointer",fontFamily:"inherit"}}>■ stop</button>
          </div>
        );
      })()}
      ...
      {page==="tasks"
        ? <TasksPage levels={LEVELS} done={done} getLive={getLive} activeTimer={activeTimer} toggleDone={toggleDone} startTimer={startTimer} stopTimer={stopTimer} openLv={openLv} toggleLv={toggleLv}/>
        : <StatsPage done={done} getLive={getLive} totalSecs={totalSecs}/>
      }
    </div>
  );
}
...
// ─── TASKS PAGE ───────────────────────────────────────────────────────────────
function TasksPage({levels,done,getLive,activeTimer,toggleDone,startTimer,stopTimer,openLv,toggleLv}){
  return(
    <div style={{padding:"0 16px 40px"}}>
      {levels.map(lv=>{
        ...
        return(
          <div key={lv.id} style={{marginBottom:10,border:`1px solid ${isOpen?lv.color+"55":"#27272A"}`,borderRadius:10,overflow:"hidden",background:"#111113"}}>
            <div onClick={()=>toggleLv(lv.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 14px",cursor:"pointer",background:isOpen?lv.color+"11":"transparent"}}>
              <span style={{fontSize:10,fontWeight:500,color:lv.color,background:lv.color+"22",padding:"2px 8px",borderRadius:20,border:`1px solid ${lv.color}44`,letterSpacing:"0.06em"}}>{lv.tag}</span>
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:500,color:"#FAFAFA"}}>{lv.title}</div>
                <div style={{fontSize:10,color:"#71717A",marginTop:1}}>{lv.subtitle}</div>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:12,color:lv.color,fontWeight:500}}>{lvPct}%</div>
                {lvSecs>0&&<div style={{fontSize:10,color:"#52525B"}}>{fmtHuman(lvSecs)}</div>}
              </div>
              <span style={{fontSize:12,color:"#52525B"}}>{isOpen?"▴":"▾"}</span>
            </div>
            <div style={{height:2,background:"#27272A"}}>
              <div style={{height:"100%",width:`${lvPct}%`,background:lv.color,transition:"width .4s"}}/>
            </div>
            {isOpen&&(
                  ...
                  return(
                    <div key={cat.name} style={{marginBottom:12}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                        <span style={{fontSize:10,color:"#52525B",letterSpacing:"0.08em",textTransform:"uppercase",whiteSpace:"nowrap"}}>{cat.name}</span>
                        <span style={{fontSize:10,color:"#3F3F46"}}>{catDone}/{cat.tasks.length}</span>
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:4}}>
                        {cat.tasks.map(t=>{
                          ...
                          return(
                            <div key={t.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",borderRadius:7,background:"#18181B",border:`1px solid ${isActive?"#166534":isDone?"#27272A":"#27272A"}`,transition:"border-color .2s"}}>
                              <div onClick={()=>toggleDone(t.id)} style={{width:17,height:17,borderRadius:4,border:`1.5px solid ${isDone?lv.color:"#3F3F46"}`,background:isDone?lv.color:"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,transition:"all .2s"}}>
                                {isDone&&<span style={{fontSize:10,color:"#fff",lineHeight:1}}>✓</span>}
                              </div>
                              <span style={{flex:1,fontSize:12,color:isDone?"#52525B":"#D4D4D8",textDecoration:isDone?"line-through":"none"}}>{t.name}</span>
                              <span style={{fontSize:11,color:isActive?"#22C55E":liveTime>0?"#71717A":"#3F3F46",fontVariantNumeric:"tabular-nums",minWidth:44,textAlign:"right"}}>
                                {isActive?fmtTimer(liveTime):liveTime>0?fmtHuman(liveTime):"—"}
                              </span>
                              <button onClick={()=>isActive?stopTimer(t.id):startTimer(t.id)} style={{fontSize:11,padding:"3px 9px",borderRadius:5,border:`1px solid ${isActive?"#166534":"#3F3F46"}`,background:isActive?"#052e16":"transparent",color:isActive?"#22C55E":"#71717A",cursor:"pointer",fontFamily:"inherit",flexShrink:0}}>
                                {isActive?"■":"▶"}
                              </button>
                            </div>
                          );
...
// ─── STATS PAGE ───────────────────────────────────────────────────────────────
function StatsPage({done,getLive,totalSecs}){
  ...
  return(
    <div style={{padding:"0 16px 40px"}}>
      <div style={{marginBottom:16}}>
        <div style={{fontSize:10,color:"#52525B",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:10}}>Level breakdown</div>
        {LEVELS.map(lv=>{
          const allT=lv.categories.flatMap(c=>c.tasks);
          const lvDone=allT.filter(t=>done[t.id]).length;
          const lvSecs=allT.reduce((a,t)=>a+getLive(t.id),0);
          const lvPct=allT.length?Math.round(lvDone/allT.length*100):0;
          return(
            <div key={lv.id} style={{marginBottom:8,background:"#18181B",border:"1px solid #27272A",borderRadius:8,padding:"10px 12px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:10,color:lv.color,background:lv.color+"22",padding:"1px 7px",borderRadius:20,border:`1px solid ${lv.color}33`}}>{lv.tag}</span>
                  <span style={{fontSize:12,color:"#D4D4D8"}}>{lv.title}</span>
                </div>
                <div>
                  <span style={{fontSize:12,color:lv.color,fontWeight:500}}>{lvPct}%</span>
                  <span style={{fontSize:11,color:"#52525B",marginLeft:8}}>{fmtHuman(lvSecs)}</span>
                </div>
              </div>
              <div style={{height:4,background:"#27272A",borderRadius:2}}>
                <div style={{height:"100%",width:`${lvPct}%`,background:lv.color,borderRadius:2,transition:"width .4s"}}/>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",marginTop:5}}>
                <span style={{fontSize:10,color:"#52525B"}}>{lvDone} of {allT.length} tasks</span>
                <span style={{fontSize:10,color:"#52525B"}}>{fmtHuman(lvSecs)} logged</span>
              </div>
            </div>
          );
        })}
      ...
      {topTasks.length>0&&(
        <div style={{marginBottom:16}}>
          <div style={{fontSize:10,color:"#52525B",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:10}}>Most time spent</div>
          <div style={{background:"#18181B",border:"1px solid #27272A",borderRadius:8,padding:"10px 12px",display:"flex",flexDirection:"column",gap:8}}>
            {topTasks.map(t=>{
              const barW=Math.round((t.secs/maxSecs)*100);
              const lvColor=LEVELS.find(l=>l.categories.some(c=>c.tasks.some(tk=>tk.id===t.id)))?.color||"#6D28D9";
              return(
                <div key={t.id}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                    <span style={{fontSize:11,color:done[t.id]?"#52525B":"#A1A1AA",textDecoration:done[t.id]?"line-through":"none",maxWidth:"75%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.name}</span>
                    <span style={{fontSize:11,color:"#71717A",fontVariantNumeric:"tabular-nums"}}>{fmtHuman(t.secs)}</span>
                  </div>
      ...
      <div style={{background:"#18181B",border:"1px solid #27272A",borderRadius:8,padding:"12px 14px"}}>
        <div style={{fontSize:10,color:"#52525B",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:8}}>Total</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {[["Total tasks",ALL_TASKS.length],["Completed",ALL_TASKS.filter(t=>done[t.id]).length],["Time logged",fmtHuman(totalSecs)],["XP earned",ALL_TASKS.filter(t=>done[t.id]).length*100]].map(([k,v])=>(
            <div key={k} style={{background:"#111113",border:"1px solid #27272A",borderRadius:6,padding:"8px 10px"}}>
              <div style={{fontSize:10,color:"#52525B",marginBottom:2}}>{k}</div>
              <div style={{fontSize:15,fontWeight:500,color:"#E4E4E7"}}>{v}</div>
            </div>
          ))}
        </div>
      </div>
