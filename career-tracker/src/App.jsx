...
export default function App() {
  ...
  const startStop = useCallback((taskId) => {
    if (timerRef.current.id === taskId) {
      ...
      update(d => {
        ...
        d.categories.forEach(c => c.levels && c.levels.forEach(l => l.tasks && l.tasks.forEach(t => {
  ...
  const getLive = useCallback((taskId) => {
    ...
    const allTasks = data.categories.flatMap(c => c.levels ? c.levels.flatMap(l => l.tasks || []) : []);
  ...
  if (!data || !data.categories) return <Splash text="loading your data..." />;
  ...
  return (
    <div style={{ fontFamily: "'Azeret Mono','Courier New',monospace", background: "#080808", minHeight: "100vh", color: "#E8E6E1" }}>
      ...
      {/* ACTIVE TIMER BANNER */}
      {activeTimer && (() => {
        const task = data.categories.flatMap(c => c.levels ? c.levels.flatMap(l => l.tasks || []) : []).find(t => t.id === activeTimer);
        return (
...
function TrackerPage({ cat, activeTimer, getLive, toggleDone, startStop, update }) {
  const [openLv, setOpenLv] = useState({});
  
  useEffect(() => {
    if (cat && cat.levels && cat.levels[0]) {
      setOpenLv(p => ({ ...p, [cat.levels[0].id]: true }));
    }
  }, [cat]);

  const levels = cat?.levels || [];
  const allTasks = levels.flatMap(l => l.tasks || []);
  ...
  return (
    <div>
      ...
      {levels.map(lv => {
        ...
        const lvDone = tasks.filter(t => t.done).length;
        const lvPct = tasks.length ? Math.round(lvDone / tasks.length * 100) : 0;
        const lvSecs = tasks.reduce((a, t) => a + getLive(t.id), 0);
            ...
            {isOpen && (
              <div style={{ padding:"8px 12px 12px", display:"flex", flexDirection:"column", gap:4 }}>
                ...
                <AddInline placeholder="+ add task" color={cat.color} onAdd={(name) => update(d => {
                  const activeCategoryObj = d.categories.find(c => c.id === cat.id);
                  const l = activeCategoryObj?.levels?.find(l => l.id === lv.id);
                  if (l) {
                    if (!l.tasks) l.tasks = [];
                    l.tasks.push({ id: uid(), name, done: false, time: 0 });
                  }
                  return d;
                })} />
              </div>
      ...
      <AddInline placeholder="+ add level" color={cat.color} big onAdd={(name) => update(d => {
        ...
        if (c) {
          ...
          c.levels.push({ id: uid(), name, subtitle: "", tasks: [] });
        }
        return d;
      })} />
    </div>
  ...
  return (
    <div>
      ...
      {categories.map(cat => {
        ...
        const taskCount = catLevels.flatMap(l => l.tasks || []).length;
        ...
        return (
          <div key={cat.id} style={{ marginBottom:8, border:"1px solid #1A1A1A", borderRadius:10, overflow:"hidden", background:"#0D0D0D" }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 12px" }}>
              ...
              <span style={{ fontSize:10, color:"#2A2A2A" }}>{taskCount} tasks</span>
...
function StatsPage({ data, getLive }) {
  const categories = data?.categories || [];
  const allTasks = categories.flatMap(c => (c.levels || []).flatMap(l => (l.tasks || []).map(t => ({ ...t, catName:c.name, catColor:c.color, liveTime:getLive(t.id) }))));
  ...
  return (
    <div>
      ...
      {categories.map(cat => {
        const catLevels = cat.levels || [];
        const tasks = catLevels.flatMap(l => l.tasks || []).map(t => ({ ...t, liveTime:getLive(t.id) }));
        ...
        return (
          <div key={cat.id} style={{ marginBottom:8, background:"#0D0D0D", border:"1px solid #141414", borderRadius:8, padding:"10px 12px" }}>
            ...
            <div style={{ fontSize:10, color:"#2A2A2A", marginTop:4 }}>{done} of {tasks.length} tasks · {catLevels.length} levels</div>
          </div>
        );
      })}
...
function Splash({ text }) {
  return (
    <div style={{ background:"#080808", minHeight:"100vh", display:"flex", alignItems:"center", fontFamily:"Azeret Mono,monospace", color:"#333", fontSize:12, justifyContent:"center" }}>
      {text}
    </div>
  );
}
